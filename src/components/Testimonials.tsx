...
export default function Testimonials() {
  ...
  return (
    <section ref={ref} className="py-28 relative overflow-hidden">
      ...
      <div className="absolute -top-40 -right-40 w-[500px] h-[500px] rounded-full opacity-[0.03]" style={{ background: 'radial-gradient(circle, #F59E0B, transparent)' }} />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        <motion.div variants={containerVariants} initial="hidden" animate={inView ? 'visible' : 'hidden'} className="grid md:grid-cols-3 gap-6">
          {REVIEWS.map((review, idx) => (
            <motion.div key={review.name} variants={cardVariants} whileHover={{ y: -5 }} className="premium-card p-6">
              <div className="flex gap-1 mb-4">
                {[...Array(5)].map((_, i) => (
                  <motion.div key={i} animate={inView ? { scale: [0, 1.15, 1] } : {}} transition={{ delay: 0.3 + i * 0.08 + idx * 0.04 }}>
                    <HiStar className={`w-4 h-4 ${i < review.rating ? 'text-accent' : 'text-gray-200'}`} />
                  </motion.div>
                ))}
              </div>
              <blockquote className="mb-4">
                <p className="text-secondary/70 text-sm leading-relaxed italic border-l-2 border-accent/15 pl-4">&ldquo;{review.text}&rdquo;</p>
              </blockquote>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-[11px] text-secondary/40">
                      ...
                      <span className="text-accent/70 font-medium">{review.service}</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.8 }} className="text-center mt-12">
          <motion.div
            ...
            style={{ background: 'linear-gradient(135deg, rgba(245,158,11,0.05), rgba(245,158,11,0.015))', border: '1.5px solid rgba(245,158,11,0.12)' }}
          >
            <div className="flex text-accent">
              {[...Array(5)].map((_, i) => (
                <motion.div key={i} animate={{ rotate: [0, -4, 4, 0] }} transition={{ delay: i * 0.08, duration: 0.3 }}>
                  <HiStar className="w-5 h-5" />
                </motion.div>
              ))}
            </div>
            <div className="text-left">
              <p className="heading-md text-primary">4.8 sobre 5</p>
              <p className="body-sm">Basado en 18 opiniones de clientes</p>
            </div>
          </motion.div>
        </motion.div>
      </div>
